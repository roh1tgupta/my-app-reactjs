...
function Resource ({ match }) {
  const topic = topics.find(({ id }) => id === match.params.topicId)
    ...
    console.log('inside resource url',match.url);
    console.log('inside resource path',match.path);
  return (
    <div>
      <h3>{topic.name}</h3>
    ...
    <div>
      ...
      <ul>
        {topic.resources.map((sub) => (
          <li key={sub.id}>
          {console.log('inside topic url',match.url)}
          {console.log('inside topic path',match.path)}
  ...
  return (
    <div>
      ...
      <ul>
        {topics.map(({ name, id }) => (
          <li key={id}>
          {console.log('inside topics url',match.url , ' id ', id)}
          {console.log('inside topics path',match.path)}
